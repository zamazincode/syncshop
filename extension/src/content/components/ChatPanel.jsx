...
const MessageItem = memo(({ msg, isMe, isBot, isMentioned }) => {
      ...
      <div
        className={`max-w-[85%] px-3.5 py-2.5 rounded-xl text-xs leading-relaxed break-words font-light ${isMe
          ...
          : isBot
            ...
            : isMentioned ? 'bg-primary/80 text-white rounded-tl-sm border border-primary/50 shadow-sm' : 'bg-white/5 text-white/90 rounded-tl-sm border border-white/5'
          }`}
        dangerouslySetInnerHTML={{ __html: renderMarkdown(msg.text) }}
      />
    </div>
  );
}, (prev, next) => prev.msg.text === next.msg.text && prev.msg.from === next.msg.from && prev.isMentioned === next.isMentioned);
    ...
    <div className="flex flex-col h-full">
      ...
      <div ref={containerRef} className="flex-1 overflow-y-auto px-5 py-4 space-y-4 ss-scrollbar">
        ...
        {messages?.map((msg, i) => {
          ...
          const isMentioned = !isMe && msg.text && msg.text.includes(`@${userName}`);
          return <MessageItem key={msg.id || i} msg={msg} isMe={isMe} isBot={isBot} isMentioned={isMentioned} />;
        })}
